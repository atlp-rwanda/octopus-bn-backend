...
class accommodation {
  static async create(req, res) {
    try {
      const {
        body: { name, country, city, imageUrl, amenities, around },
  ...
  static async addRoom(req, res) {
    try {
      const {
        body: { accommodationsID, roomNumber, cost, currency, type },
        user: { id, preferedLang },
      } = req;
    ...
    try {
      ...
      if (!findRequest) {
        return errorResponse(
          res,
          404,
          setLanguage(preferedLang).__("noRequestFound")
        );
      }
      ...
      if (findRequest.status !== "pending") {
        return errorResponse(
          res,
          403,
          setLanguage(preferedLang).__("rejected")
        );
      }

      const to = findRequest.to.split(" - ")[1];
      const accommodations = await bookingService.getAccommodations(
        to,
        page,
        limit
      );
      if (accommodations === null) {
        return errorResponse(
          res,
          404,
          setLanguage(preferedLang).__("NoAccommondation")
        );
      }
      ...
      return successResponse(
        res,
        200,
        setLanguage(preferedLang).__("accommodationsPerDest"),
        accommodations
      );
    } catch (error) {
  ...
  static async bookAccommodation(req, res) {
    try {
      const {
        body: { accommodationId, roomId, tripId, checkIn, checkOut },
        user: { preferedLang, id },
      } = req;
      ...
      await Rooms.update({ status: false }, { where: { id: roomId } });

      return successResponse(
        res,
        201,
        setLanguage(preferedLang).__("bookedSuccess")
      );
    } catch (error) {
      return errorResponse(res, 500, error.message);
    }
    ...
    try {
      ...
      if (data.length <= 0) {
        return errorResponse(
          res,
          404,
          setLanguage(preferedLang).__("AccommodationNotFound")
        );
      }
      ...
      return successResponse(
        res,
        201,
        setLanguage(preferedLang).__("ThankFeedback")
      );
    } catch (error) {
    ...
    try {
      ...
      if (!data) {
        return errorResponse(
          res,
          404,
          setLanguage(preferedLang).__("notFoundAccommodation")
        );
      }
      ...
      if (preRatings) {
        ...
        return successResponse(
          res,
          200,
          setLanguage(preferedLang).__("thanksYouAgain")
        );
      }
      ...
      return successResponse(
        res,
        201,
        setLanguage(preferedLang).__("ThanksYouRating")
      );
    } catch (error) {
      return res.status(error.status || 500).json({
        errors: {
  ...
  static async LikeOrUnlike(req, res) {
    try {
      ...
      const liked = await AcommodationLikesAndUnlikes.findOne({
        where: { userId: id, accommodationId },
      });
      const like = {
        id: uuid(),
      ...
      await AcommodationLikesAndUnlikes.destroy({
        where: { userId: id, accommodationId },
      });
      return res.status(200).json({
        status: 200,
  ...
  static async getMostTravelledCentres(req, res) {
    try {
      ...
      const mostTraveledCentres = await bookingService.getTrendingCentres(
        page,
        limit
      );
      ...
      return successResponse(
        res,
        200,
        setLanguage(preferedLang).__("trendingDestinations"),
        mostTraveledCentres
      );
    } catch (error) {
      return errorResponse(res, 500, error.message);
  ...
  static async getAllAccommodations(req, res) {
    try {
      ...
      const mostTraveledCentre = await bookingService.getTrendingCentres(
        page,
        limit
      );
      return successResponse(
        res,
        200,
        setLanguage(preferedLang).__("allAccommodations"),
        { allAccommodations, mostTraveledCentre }
      );
    } catch (error) {
      return errorResponse(res, 500, error.message);
    }
  ...
  static async searchAccommodations(req, res) {
    try {
      const {
        query: { page, limit, searchKey },
        user: { preferedLang },
      } = req;
      const results = await accommodationService.searchResults(
        page,
        limit,
        searchKey
      );
      ...
      return successResponse(
        res,
        200,
        setLanguage(preferedLang).__("results"),
        results
      );
    } catch (error) {
      return errorResponse(res, 500, error.message);
    ...
    try {
      ...
      const result = await Accommodations.findOne({
        ...
        include: [
          ...
          },
          {
            model: Feedbacks,
            attributes: {
              exclude: ["accommodationId", "userId", "createdAt", "updatedAt"],
            },
            include: [
              {
                model: Users,
      ...
      if (!result) {
        return errorResponse(
          res,
          404,
          setLanguage(preferedLang).__("accommodationIdNotFound")
        );
      }
      ...
      return successResponse(
        res,
        200,
        setLanguage(preferedLang).__("oneAccommodation"),
        result
      );
    } catch (error) {
      return errorResponse(res, 500, error.message);
